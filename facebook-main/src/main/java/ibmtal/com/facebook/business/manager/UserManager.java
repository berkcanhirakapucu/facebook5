...
public class UserManager implements UserService {
	...
	public Result newUser(User user) {
		...
		if(user.getName().isBlank()) {
			result.setSuccess(false);
			mesaj.add(new Message("Name","boş geçilemez.en az 3 karakter olmalı."));
		}
		if(user.getSurname().isBlank()) {
			result.setSuccess(false);
			mesaj.add(new Message("Surname","boş geçilemez.en az 2 karakter olmalı."));
		}
		if(result.isSuccess()) {
			userDao.save(user);
		}
	...
	public Result userAdd(UserAdd userAdd) {
		...
		user.setPassword(userAdd.getRePassword());
